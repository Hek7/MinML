...
The finished touches are to write a suite of tests for th test_that folder and add to the functions with more optimizers and losses. 
...
# Backwatd pass to get the grad loss 
...
cat("Gradient computed for first layer weights:", !is.null(all_grads[[1]]$dW), "\n")
```
...
for (epoch in 1:epochs) {
  ...
  for (i in seq_along(model$modules)) {
    layer <- model$modules[[i]]
    
    # Check if the module has parameters (like Linear layers)
    if ("params" %in% names(layer)) {
      
      # Perform SGD update
      # Returns a list of these new params 
      updated_params <- sgd_update_params(
        current_params = layer$params,
        gradients = layer$grads,
        learning_rate = learning_rate
      )
      
      # Quirk of the current package is that we have to update these manually 
      # This is because R lists are like copies and not pass by reference 
      #   To be fixed in the future 
      model$modules[[i]]$params <- updated_params
    }
  }
